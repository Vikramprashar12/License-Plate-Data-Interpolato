...
#             draw_border(frame, (int(car_x1), int(car_y1)), (int(car_x2), int(car_y2)), (0, 255, 0), 10,  # Adjusted thickness
...
#                 y2)), (0, 0, 255), 5)  # Adjusted thickness
...
while ret:
    ...
    if ret:
        ...
        for row_indx in range(len(df_)):
            ...
            if license_number:
                (text_width, text_height), _ = cv2.getTextSize(
                    license_number, cv2.FONT_HERSHEY_SIMPLEX, 1.2, 3)

                # Ensure the coordinates are integers
                # Align text to the left side of the license plate bounding box
                text_x = int(x1)
                # Place text below the bounding box with a slight offset
